...
public abstract class GameObject {
	...
	/**
	 * Each subclass of GameObject should also implement a function that looks something like this:
	 * <code>
	 * public static Class<? extends GameObject>[] getCollisionList(){
		Class<?>[] list = {Enemy.class, Wall.class};
		return (Class<? extends GameObject>[]) list;
	}
	 * </code>
	 * 
	 * It returns a list of all objects that this object has collision events with.
	 * (Actually, only one class per pair of classes that collide needs to do include the other in its collisionList)
	 * It really should have a "final" modifier, but I can't figure out how to make something protected, static, and final all at the same time
	 * Define it in all classes that extend GameObject by overriding <code>getCollsionList()</code>.
	 * If this GameObject has no collision events with other objects, define this to an empty ArrayList.
	 */
	protected static Class<?>[] collisionList; // Rethink the "protected" modifier, and see if you can't figure out a better way to implement this WITHOUT reflection
	...
	*/
	...
	public void draw(Graphics g){
		...
		//MiscUtilities.drawDot(g, motion.position());
	}
